...
/**
* Add a custom meta box class
* 
* @package JmraFramework
* @param array $meta_box Meta box input data
*/


        add_action( 'admin_enqueue_scripts',  'jmra_metabox_scripts' );
        add_action( 'wp_ajax_stag_save_images','jmra_save_images'  );
        add_action( 'save_post',  'jmra_save_meta_box'  );

	function jmra_add_meta_box( $meta_box ) {
		if( !is_array( $meta_box) ) return false;
	
	$callback = create_function( '$post,$meta_box', 'jmra_create_meta_box( $post, $meta_box["args"] );' );
	 ...
	 * @param array $meta_box Meta box input data
	 */
	function jmra_create_meta_box( $post, $meta_box ) {
		if( !is_array( $meta_box) ) return false;
		...
		wp_nonce_field( basename(__FILE__), 'jmra_meta_box_nonce' );
		...
		foreach( $meta_box['fields'] as $field ){
			...
			switch( $field['type'] ){
				...
				case 'images':
					...
					$thumbs_output = '';
					$button_text = ($meta) ? __('Edit Picture', 'stag') : $field['std'];
					if( $meta ) {
						$field['std'] = __('Edit Picture', 'stag');
						$thumbs = explode(',', $meta);
						$thumbs_output = '';
	 ...
	 */
	function jmra_save_meta_box( $post_id ) {

		if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) 
			return;

		if ( !isset($_POST['stag_meta']) || !isset($_POST['jmra_meta_box_nonce']) || !wp_verify_nonce( $_POST['jmra_meta_box_nonce'], basename( __FILE__ ) ) )
			return;
	...
	/**
	 * Save custom meta box image
	 *
	 * @param int $post_id The post ID
	 */
	function jmra_save_images(){
		if( defined('DOING_AUTOSAVE') && DOING_AUTOSAVE ){
			return;
		}
